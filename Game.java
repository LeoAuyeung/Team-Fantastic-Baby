...
public class Game {
    ...
    //have a global var battleScreen to check what screen user is at during a game
    //executeBattleControl will execute depending on what screen it is
    //stay on same screen unless press B or command is executed
	...
	// Sungbin: I think this should be implemented in each of other display method because of Battle.reset()
	...
	Battle.set( 2, 0, displayHP( enemyPokemon ) + " (" + enemyPokemon.getCurrentHP() + "/" + enemyPokemon.getMaxHP() + ")" );
	...
	Battle.set( 6, 0, displayHP( currentPokemon ) + " (" + currentPokemon.getCurrentHP() + "/" + currentPokemon.getMaxHP() + ")" );
	...
	//display Pokemon's list of available moves
    }
	
    public void displayBag() {
	//display user's list of bags; 1: Pokeballs, 2: Potions, 3: Battle Items
    }

    // helper methods to equally space out the first column of "displayPokemon()"
	...
	Battle.set( 10, 0, "------------------------------ ------------------------------" );
	Battle.set( 11, 0, indent(" (1) Lv." + _Pokemon.get(0).getLevel() + "   " + _Pokemon.get(0).getName() + "(" + _Pokemon.get(0).getType() + ")") );
	Battle.set( 12, 0, indent(displayHP( _Pokemon.get(0) )) );
	Battle.set( 13, 0, "------------------------------ ------------------------------" );
	Battle.set( 14, 0, indent(" (2) Lv." + _Pokemon.get(1).getLevel() + "   " + _Pokemon.get(1).getName() + "(" + _Pokemon.get(1).getType() + ")") );
	Battle.set( 15, 0, indent(displayHP( _Pokemon.get(1) )) );
	Battle.set( 16, 0, "------------------------------ ------------------------------" );
	Battle.set( 17, 0, indent(" (3) Lv." + _Pokemon.get(2).getLevel() + "   " + _Pokemon.get(2).getName() + "(" + _Pokemon.get(2).getType() + ")") );
	Battle.set( 18, 0, indent(displayHP( _Pokemon.get(2) )) );
	Battle.set( 19, 0, "------------------------------ ------------------------------" );
	// second column
	Battle.set( 11, 1, " " + indent(" (4) Lv." + _Pokemon.get(3).getLevel() + "   " + _Pokemon.get(3).getName() + "(" + _Pokemon.get(3).getType() + ")") );
	Battle.set( 12, 1, " " + indent(displayHP( _Pokemon.get(3) )) );
	Battle.set( 14, 1, " " + indent(" (5) Lv." + _Pokemon.get(4).getLevel() + "   " + _Pokemon.get(4).getName() + "(" + _Pokemon.get(4).getType() + ")") );
	Battle.set( 15, 1, " " + indent(displayHP( _Pokemon.get(4) )) );
	Battle.set( 17, 1, " " + indent(" (6) Lv." + _Pokemon.get(5).getLevel() + "   " + _Pokemon.get(5).getName() + "(" + _Pokemon.get(5).getType() + ")") );
	Battle.set( 18, 1, " " + indent(displayHP( _Pokemon.get(5) )) );
    }
	...
	double eachBar = ( (p.getLevelEXP())/20.0 ); // this is how much hp each bar is worth
	...
	// *** FOR ALL EXCEPT RUN, HAVE A "B: BACK" IN THE BOTTOM CORNER OF DISPLAY TO ALLOW USER TO GO BACK ***
		
	if ( command.equals("1") ) { //FIGHT
    ...
    */
	...
	while ( user.getQuest() != 20 ) {
	    ...
	    //Displays Screen
	    //thinking of implementing displayBattle way of displaying stuff into map's way so that
	    //we can display player's profile, pokemon, bag, etc
